...
object Entrance extends App {
  ...
  override def main(args: Array[String]) {
    val spark = SparkSession
      .builder()
      .appName("Project2-HotspotAnalysis-MYGROUPNAME") // YOU NEED TO CHANGE YOUR GROUP NAME
  ...
  private def queryLoader(spark: SparkSession, queryName: String, queryParams: String, outputPath: String) {
    ...
    if (queryName.equalsIgnoreCase("hotcellanalysis")) {
      if (queryParam.length != 1) throw new ArrayIndexOutOfBoundsException("[Project2] Query " + queryName + " needs 1 parameters but you entered " + queryParam.length)
      ...
      if (queryParam.length != 2) throw new ArrayIndexOutOfBoundsException("[Project2] Query " + queryName + " needs 2 parameters but you entered " + queryParam.length)
      HotzoneAnalysis.runHotZoneAnalysis(spark, queryParam(0), queryParam(1)).write.mode(SaveMode.Overwrite).csv(outputPath)
    }
    else {
      throw new NoSuchElementException("[Project2] The given query name " + queryName + " is wrong. Please check your input.")
    }
  }
}
